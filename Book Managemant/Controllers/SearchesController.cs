...
namespace Book_Managemant.Controllers
{
    public class SearchesController : Controller
    {
        ...
        {
            var searches = db.Searches.Include(s => s.Author)
                                      .Include(s => s.Category)
                                      .Include(s => s.Publisher);

            if (searchBy == "Name")
            {
                searches = searches.Where(s => s.Author.Name.StartsWith(search) || search == null);
            }
            else if (searchBy == "CategoryName")
            {
                searches = searches.Where(s => s.Category.CategoryName.StartsWith(search) || search == null);
            }
        ...
        public ActionResult Create([Bind(Include = "bookID,AuthorID,CategoryID,PublisherID")] Search search)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "bookID,AuthorID,CategoryID,PublisherID")] Search search)
        {
            if (ModelState.IsValid)
            {
